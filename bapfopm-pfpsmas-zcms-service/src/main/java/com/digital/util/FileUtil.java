...
public class FileUtil {
    ...
    private static final String entityPath = "E:/YLQHworkspace/ZoningCodeFB/";
    ...
    private static final String QHDM_BGMX_TITLE = "调整编号,调整说明,原区划名称,原区划代码,调整类型,现区划名称,现区划代码";
...
//                   if ("XZQH".equals(txtName.substring(txtName.lastIndexOf("/") + 1, txtName.indexOf("_")))) {
    ...
    public static <X> void writerTxt(List<X> list, String filePath,X name){
        File file = null;
        ...
        try {
            ...
            buffer.clear();
            if (name instanceof FormalTableInfo) {
                List<FormalTableInfo> formalTableInfoList = (List<FormalTableInfo>) list;
                //第一行为数据数量   第二行为列名  第三行以及之后的为数据
                String check = String.valueOf(list.size()) + "\r\n";
                byte[] bytes = check.getBytes();
                buffer.put((String.valueOf(list.size()) + "\r\n").getBytes());
                //区划名称,区划代码
                buffer.put((QHDM_TITLE + "\r\n").getBytes());
                buffer.flip();
                channel.write(buffer);
                for (FormalTableInfo v : formalTableInfoList) {
                    buffer.clear();
                    buffer.put((v.getDivisionName() + sign + v.getZoningCode() + "\r\n").getBytes());
                    buffer.flip();
                    channel.write(buffer);
                }
            } else if (name instanceof ZCCDetail) {
                List<ZCCDetail> zccDetailList = (List<ZCCDetail>) list;

                buffer.put((String.valueOf(list.size()) + "\r\n").getBytes());
                //"调整编号,调整说明,原区划名称,原区划代码,调整类型,现区划名称,现区划代码"
                buffer.put((QHDM_BGMX_TITLE + "\r\n").getBytes());
                buffer.flip();
                channel.write(buffer);
                for (ZCCDetail zccDetail : zccDetailList) {
                    buffer.clear();
                    buffer.put((zccDetail.getGroupSeq() + sign +zccDetail.getGroupName()+sign +
                            zccDetail.getOriginalZoningName() + sign + zccDetail.getOriginalZoningCode() + sign +
                            zccDetail.getChangeType() + sign +
                            zccDetail.getCurrentZoningName() + sign + zccDetail.getCurrentZoningCode() + sign +
                            "\r\n").getBytes());
                    buffer.flip();
                    channel.write(buffer);
                }
            } else if (name instanceof ContrastTemporary) {
                List<ContrastTemporary> temporaryList = (List<ContrastTemporary>) list;
                //"调整编号,调整说明,原区划名称,原区划代码,调整类型,现区划名称,现区划代码,错误信息"

                buffer.put((String.valueOf(list.size()) + "\r\n").getBytes());
                buffer.put((QHDM_BGDZB_TITLE + "\r\n").getBytes());
                buffer.flip();
                channel.write(buffer);
                for (ContrastTemporary temporary : temporaryList) {
                    buffer.clear();
                    buffer.put((temporary.getGroupNum() + sign + temporary.getGroupName() + sign +
...
//    }
    ...
    * */
     ...
     * @description 压缩文件
     * @method  toZip
     * @params  path : 被压缩的路径； zipPath : 压缩后zip文件的存储路径
     * @return  boolean true 成功；false 失败
     * @exception
     */
    public static boolean toZip(File file,String zipPath){
        boolean check = true;
        ...
        try {
            ...
            logger.error("递归压缩文件的时候出来错误！"+e.getMessage());
            e.printStackTrace();
            check = false;
        }
        return check;
    ...
    /**
     * @description 压缩文件基础方法
     * @method  compress
     * @params sourceFile ：要压缩的文件路径；zos ：要保存压缩文件的地址；KeepDirStructure : 是否保存文件原有的结构
     * @return
     * @exception Exception
     */
