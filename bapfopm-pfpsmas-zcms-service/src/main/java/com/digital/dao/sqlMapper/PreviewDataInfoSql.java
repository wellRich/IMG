...
public class PreviewDataInfoSql implements BaseDao<PreviewDataInfo> {
        ...
        String sql1 = new SQL(){
            {
              ...
              FROM(entitySql.getTableName());
                WHERE(entitySql.getColumnByField("chooseSign") + "= 'Y' AND " + entitySql.getColumnByField("usefulSign") + "= 'Y'");
                AND();
    ...
    public String findAncestorsAndSubsByZoningCode(String zoningCode) {
        ...
        log.info("findAncestorsAndSubsByZoningCode.processedCodes----> " + processedCodes);
        sql = new SQL() {{
            FROM(entitySql.getTableName());
            SELECT(entitySql.getColumns());
            WHERE(entitySql.getColumnByField("chooseSign") + "= 'Y' AND " + entitySql.getColumnByField("usefulSign") + "= 'Y'");
    ...
    public String findValidOneByZoningCode(String zoningCode){
        String sql = new SQL(){{
            FROM(entitySql.getTableName());
            String COLUMNS = entitySql.getColumnsExcPrimary().replace("UNIQUE_KEY,","DISTINCT ");
            SELECT(COLUMNS);
