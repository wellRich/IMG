...
public class ZCCRequestSql implements BaseDao<ZCCRequest> {
    ...
    public String updateDetailByStatus(String status){
        String sql = new SQL(){
            {
                UPDATE(entitySql.getTableName());
                SET(entitySql.getColumnByField("status") +"='50'");
                WHERE(entitySql.getColumnByField("status") +"= '"+status+"'");
            }
        }.toString();
        log.info("updateDetailByStatus----------->"+sql);
        return sql;
    }
