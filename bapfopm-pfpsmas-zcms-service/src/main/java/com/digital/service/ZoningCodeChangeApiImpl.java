...
public class ZoningCodeChangeApiImpl implements ZoningCodeChangeApi {
    ...
    private QueryResp<ZCCRequest> pageSeekByLevelCodeAndStatuses(String levelCode, Integer pageIndex, Integer pageSize, Integer total){
        ...
        resp.query(() -> pageSeekByLevelCodeAndStatuses(levelCode, resp.getOffset(), pageSize, Common.XZQH_SQDZT_SHBTG, Common.XZQH_SQDZT_WTJ));
        if(total == 0){
            resp.count(() -> zccRequestMapper.countByZoningCodeAndStatus(levelCode, Common.XZQH_SQDZT_SHBTG, Common.XZQH_SQDZT_WTJ));
        }else {
