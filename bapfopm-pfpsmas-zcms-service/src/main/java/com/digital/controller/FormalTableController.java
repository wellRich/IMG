...
public class FormalTableController extends BaseController {
    ...
    /**
     * @description 压缩区划代码
     * @method  releaseFormalTable
     * @params
     * @return
     * @exception
     */

    @RequestMapping(value = "/import/release",method = RequestMethod.GET)
    @ResponseBody
    public String releaseFormalTable(){
        boolean result = importFormalTableApi.writerZoningCodeToTxt();
        if (result)return new RtnData().toString();
        return new RtnData(Constants.RTN_CODE_ERROR,Constants.RTN_RELEASE_FAIL).toString();
